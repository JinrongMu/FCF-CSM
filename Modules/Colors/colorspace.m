...
%     'YPbPr'            Luma (ITU-R BT.601) + Chroma  Y:(0,1),Pb:(-0.5,0.5),Pr:(-0.5,0.5)
%     'YCbCr'/'YCC'      Luma + Chroma ("digitized" version of Y'PbPr)  Y:(16,235),Cb,Cr:(16,240)
%     'YUV'              NTSC PAL Y'UV Luma + Chroma  Y:(0,1),U:(-0.436,0.436),V:(-0.615,0.615)
%     'YIQ'              NTSC Y'IQ Luma + Chroma  Y:(0,1),I:(-0.5957,0.5957),Q:(-0.5226,0.5226)
%     'YDbDr'            SECAM Y'DbDr Luma + Chroma  Y:(0,1),Db,Dr:(-1.333,1.333)
%     'JPEGYCbCr'        JPEG-Y'CbCr Luma + Chroma
%     'HSV'/'HSB'        Hue Saturation Value/Brightness     H:(0,359.76),S,V:(0,1)
%     'HSL'/'HLS'/'HSI'  Hue Saturation Luminance/Intensity (HSI)  H:(0,359.76),S,L:(0,1)
%     'XYZ'              CIE XYZ
%     'Lab'              CIE L*a*b* (CIELAB)  L:(0,100),a:(-86.18,98.26), b:(-107.87,94.48)
%     'Luv'              CIE L*u*v* (CIELUV)  L:(0,100),u:(-83.08,175.05), v:(-134.12,107.39)
%     'Lch'              CIE L*ch (CIELCHab) L:(0,100),c:(0,133.82), h:(0,360)
%
...
switch Space
    ...
    case {'hsl','hsi','hls'}
        Space = 'hsl';
    case {'rgb','yuv','yiq','ydbdr','ypbpr','jpegycbcr','xyz','lab','luv','lch'}
        return;
end
return;
...
switch SrcSpace
    ...
    case {'xyz','lab','luv','lch'}
        ...
        T = [3.2406,-1.5372,-0.4986;-0.9689,1.8758,0.0415;0.0557,-0.2040,1.0570]; % wiki/SRGB
...
switch SrcSpace
    ...
    otherwise   % Convert from some gamma-corrected space
        ...
        T = [0.4124,0.3576,0.1805;0.2126,0.7152,0.0722;0.0193,0.1192,0.9505]; % wiki/SRGB
...
H = H*180/pi + 360*(H < 0);     % (-pi,0)->(pi,2pi)
...
%% Gamma Correction
%
% Matlab Documentation Contents:
%     Documentation Home >> Image Processing Toolbox >> Import, Export,and
%     Conversion >> Color >> Automatic White Balance >> lin2rgb (rgb2lin)
% References
%     [1] Ebner, Marc. "Gamma Correction." Color Constancy. Chichester, West
%         Sussex: John Wiley & Sons, 2007. 
%     [2] Adobe Systems Incorporated. "Inverting the color component transfer
%         function." Adobe RGB (1998) Color Image Encoding. Section 4.3.5.2, May 2005, p.12.
% 
% Gamma Correction Using the sRGB Standard
function Rp = gammacorrection(R)
% Equivalent to lin2rgb
    i = R > 0.0031308;
    Rp(i) = 1.055*(R(i).^(1/2.4))-0.055;
    Rp(~i) = 12.92 * R(~i);
    Rp = reshape(Rp, size(R));
return;
...
% Equivalent to rgb2lin
    i = (Rp > 0.04045);
    R(i) = ((Rp(i) + 0.055) / 1.055) .^ 2.4;
    R(~i) = Rp(~i) / 12.92;
    R = reshape(R, size(Rp));
return;
